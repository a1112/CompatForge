...
Do not claim cross-process compare-and-install atomicity: RuntimePackStore currently has only an in-process lock. Bound every manifest read at bundle install, existing-manifest publish, and installed-manifest load to 1 MiB by reading at most cap+1; reject newly serialized manifests above the same limit before active-ref publication. An install/ref failure may leave valid content-addressed object/manifest files, but it must not overwrite a pre-existing conflicting ref, emit a Context, or print a success receipt. If active-ref rename succeeds and its directory sync fails, return a typed commit-uncertain failure, require the caller to inspect the visible ref rather than automatically rolling it back, and retain whether staging cleanup also failed; immutable object/manifest directory-sync failures remain ordinary install failures. Test this exact allowed-leftover boundary. The real Runner avoids it by using a new exclusive Store root.
